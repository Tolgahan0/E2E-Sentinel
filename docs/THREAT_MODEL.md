...
| Generated test code execution | Mitigated in `docker` execution mode (Phase 5); weaker by design in `local` mode | Deterministic (no-AI) spec generation either way. In `docker` mode (the default when a Docker daemon is reachable), execution is always inside a disposable, resource-limited, non-root container — never in `sentinel-api`'s own process. `SENTINEL_EXECUTION_MODE=local` (or `auto` falling back to it) trades that away deliberately for machines with no Docker at all: the generated spec runs as a plain host process with `sentinel-api`'s own privileges, no per-run sandboxing — see docs/RUNNER_ISOLATION.md's "Local process execution mode". Never a silent substitution: `docker` requested by name never falls back; only `auto`'s fallback is silent, and it's logged either way. |
...
| Runner escape | Mitigated in `docker` execution mode (Phase 5); not applicable in `local` mode | Memory/CPU/wall-clock limits, non-root fixed user, no Docker socket inside the runner container, no volume beyond its own workspace. In `local` mode there is no container to escape from in the first place — the test process already runs at `sentinel-api`'s own privilege level, which is the point being traded away (see the "Generated test code execution" row above). |
