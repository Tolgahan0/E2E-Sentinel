...
| Flaky Tests | Project-wide test stability, computed from full run history | A test case that's flaked shows up here even if its latest run passed |
...
- **Visual regression testing**: every browser-based test run's
  ...
  stored baseline for that test case, at a per-project sensitivity
  threshold configurable from the *Projects* page. A visual change never
  fails a test on its own — pass/fail still comes only from the runner's
  exit code — it's surfaced on the *Visual Diffs* page for a human to
...
- **Failure analysis & bug reports**: a failed test run is automatically
  ...
  and exported as Markdown or JSON. A project-wide *Flaky Tests* view
  (computed live from each test case's run history, not just its last
  failure) surfaces stability across every test case that's run at
  least once — never hidden, even a single-run case shows as
  "insufficient evidence" rather than being omitted.
- **Fix proposals**: a candidate unified diff for a bug — either pasted
  manually or generated by whichever provider is routed for
  `fix_generation`. The AI-assisted path best-effort includes the actual
  source of the file(s) the Application Graph already identifies as
  implementing the affected route/service, redacted for secrets and
  size-capped (`internal/redaction`), with an honest fallback to
  evidence-only prompting when no such file can be identified. The AI
  can never apply anything itself: every proposal starts
  `pending_review`, can be tested by applying it to a disposable copy of
  the repository, and only reaches the real repository through an
  explicit `approve` followed by `apply-repository` — which re-parses
  the exact diff that was approved and can run at most once per
  proposal. A path-traversal check runs before every write, for both the
  temporary workspace and the real repository.
...
- **GitHub CI integration**: opt-in (`SENTINEL_GITHUB_CI_ENABLED`), per-
  ...
  branch **and** every currently open pull request's head commit —
  deliberately not a webhook receiver, since `sentinel-api` stays bound
  to `127.0.0.1` either way (see [docs/GITHUB_CI.md](docs/GITHUB_CI.md)).
  A new commit (branch or PR) runs every approved test case for that
  project through the exact same approval/production-safety gates as a
  manual run, then reports one commit status back — so an open PR shows
  its own check, not just the branch it'll eventually land on. The
  GitHub token is stored the same encrypted way an AI provider's API key
  is, and is never returned by the API.
...
- A Next.js panel (`apps/web`) on port `9090`: Dashboard, Audit Logs,
  ...
  Test Inventory, Runs, Visual Diffs, AI Providers, Bugs, Flaky Tests,
  and Fix Proposals are functional; the remaining nav sections are stub
  pages pointing at the phase that implements them.
